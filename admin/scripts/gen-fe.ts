...
async function renderOne(item: { theme: string; sub: string }, idx: number): Promise<void> {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const draw = `drawtext=fontfile='${esc(config.fonts.latin)}':textfile='${esc(txt)}':fontcolor=white:fontsize=68:borderw=7:bordercolor=black@0.9:shadowcolor=black@0.5:shadowx=0:shadowy=3:x=(w-text_w)/2:y=h*0.72:line_spacing=10:alpha='if(lt(t,0.2),t/0.2,1)'`;
