...
document.querySelectorAll('.tab').forEach(t=>t.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('on'));t.classList.add('on');try{localStorage.setItem('tab',t.dataset.t);}catch(e){}for(const s of['proj','canal','cal','banco','conex','config','stats','gastos'])$('#'+s).classList.toggle('hide',s!==t.dataset.t);if(t.dataset.t==='canal')loadChannel();if(t.dataset.t==='cal')loadCal();if(t.dataset.t==='banco')loadBank();if(t.dataset.t==='conex')loadConex();if(t.dataset.t==='config')loadConfig();if(t.dataset.t==='stats')loadStats();if(t.dataset.t==='gastos')loadCosts();});
